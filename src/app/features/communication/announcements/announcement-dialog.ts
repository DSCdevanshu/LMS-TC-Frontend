...
export class AnnouncementDialogComponent implements OnInit {
  ...
  protected readonly ref = inject(MatDialogRef<AnnouncementDialogComponent>);
  protected readonly data = inject<{ announcement: any }>(MAT_DIALOG_DATA);
  ...
  ngOnInit(): void {
    this.lookup.getDropdownData('GetAnnouncementCategoryDropDown').subscribe({
  ...
  save(): void {
    ...
    this.service.createAnnouncement(payload).subscribe({
      next: (res) => {
        this.notification.success('Saved', res?.message || 'Announcement created.');
        this.ref.close(true);
      },
      error: (err) => {
        this.notification.error('Failed', err?.message || 'Could not save announcement.');
